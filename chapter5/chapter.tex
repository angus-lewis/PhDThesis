...
Thus, returning to (\ref{eqn: 161222}), taking the limit as \(p\to\infty\), and using the convergence established in (\ref{eqn: aMMMMM}) and (\ref{eqn: lim 12}), then 
\begin{align}
...
\begin{cor}\label{cor: lk}
	Let \(\psi: \mathbb R\times \mathcal S \to \mathbb R\) be an arbitrary, bounded function with \(|\psi(\cdot,\cdot)|\leq F\). %Further, let \(\mathcal L(X(t)) = \sum_{k\in\mathcal K}k1(X(t)\in\mathcal D_k)\) and \(\widetilde{\bs X}(t) = (\mathcal L(X(t)),X(t),\varphi(t)))\). 
