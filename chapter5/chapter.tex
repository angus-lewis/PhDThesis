...
% \begin{align}
% 	&\mathbb P(L^{(p)}(\tau_1^{(p)}) = \ell_0-1(j\in\calS_-)+1(j\in\calS_+), \rapphase(\tau_1^{(p)}) = j
% 			 \mid \bs Y^{(p)}(0) = (\ell_0 ,\bs\alpha, i)).\nonumber
% 	%  \intertext{for \(j\in\calS_+\)}
% 	%  &\mathbb P(L^{(p)}(\tau_1^{(p)}) = \ell_0-1, \rapphase(\tau_1^{(p)}) = j
...
\end{proof}
A direct corollary of Lemma~\ref{cor: aln222222} is the convergence of the transition probabilities of the embedded process. 
...
% be the (stopping) times at which \(\{L^{(p)}(t)\}\), the level process of the QBD-RAP, changes, or the boundary is hit, or, if the process is at the boundary, the process leaves the boundary. To simplify notation, we may drop the superscript \(p\) where it is not explicitly needed. Further, let \(\{\Ydp(n)\}_{n\geq 0, n \in \mathbb Z} = \{(L^{(p)}(\tau_n^{(p)}),\rapphase^{(p)}(\tau_n^{(p)}))\}_{n\geq 0, n \in \mathbb Z}\) be the value of the level and phase of the discrete-time process embedded at the change-of-level epochs, \(\{\tau_n^{(p)}\}_{n\geq 0}\). The subscript \(\bs \alpha^{(p)}\) refers to the fact that \(\bs A^{(p)}(\tau_n^{(p)})=\bs \alpha^{(p)}\) for \(n\geq 1\). The process \(\{\Ydp(n)\}_{n\geq 0}\) is a discrete-time Markov chain, which is time-homogeneous for \(n\geq 1\). 
...
Formally, for \(n\geq 1\), consider the Laplace transform 
\begin{align}
...
Partitioning on the phases and the levels at the times \(\tau_m\), \(m=2,\dots,n-1\), and using the strong Markov property of the QBD-RAP, then (\ref{eqn: dskvnaSF}) is  
	\begin{align}
...
Thus, returning to (\ref{eqn: 161222}), taking the limit as \(p\to\infty\), and using the convergence established in (\ref{eqn: aMMMMM}) and (\ref{eqn: lim 12}), we see that  
\begin{align}
...
Returning to the right-hand side of (\ref{eqn: l whkrqvlkjbrd}), taking the limit as \(p\to\infty\), and recalling the limits in (\ref{eqn: ashverkjbvjhkhdsknacva}) and (\ref{eqn: lkHAHJFHKJ J J 22}), then 
\begin{align}
...
\subsection{Intermezzo: A domination condition}
...
\begin{align}
	...
	% \\&=\int_{t=0}^\infty e^{-\lambda t}\mathbb E\Bigg[\int_{x\in\calD_{L(t),\rapphase(t)}}\bs A(t)\bs v_{L(t),\rapphase(t)}(x)\psi(x,\rapphase(t)),\rapphase(t) )\wrt x  \mid \nonumber
...
\begin{cor}\label{cor: lk}
	Let \(\psi: \mathbb R\times \mathcal S \to \mathbb R\) be an arbitrary, bounded function with \(|\psi(\cdot,\cdot)|\leq F\). %Further, let \(\mathcal L(X(t)) = \sum_{k\in\mathcal K}k1(X(t)\in\mathcal D_k)\) and \(\widetilde{\bs X}(t) = (\mathcal L(X(t)),X(t),\rapphase(t)))\). 
...
% \begin{lem}
% 	\begin{align}
% 		&\Bigg|\mathbb E\left[\psi( \mathcal R(L(t),\bs A(t),\rapphase^{(p)}(t)),\rapphase^{(p)}(t) )   \mid \bs Y^{(p)}(0)=\bs y_0^{(p)} \right] 
% 		\\&{}-\mathbb E\left[\psi( \mathcal R(L(u),\bs A(u),\rapphase^{(p)}(u)),\rapphase^{(p)}(u) )   \mid \bs Y^{(p)}(0)=\bs y_0^{(p)}\right] \Bigg| < Lc_{max}|t-u| \nonumber
% 	\end{align}
...
% 	Suppose \(i\in\calS_-\). 
% 	\begin{align*}
% 		&\mathbb E\left[\psi( \mathcal R(L(t+u),\bs A(t+u),\rapphase^{(p)}(t+u)),\rapphase^{(p)}(t+u) )   \mid L(t)=\ell,\bs A(t)=\bs a, \rapphase^{(p)}(t)=i \right]
% 		%
% 		\\&=\mathbb E\left[\int_{x\in\calD_{L(t+u),\rapphase^{(p)}(t+u)}}\bs A(t+u)\bs v_{L(t+u),\rapphase^{(p)}(t+u)}\psi(x,\rapphase^{(p)}(t+u) )\wrt x   \mid L(t)=\ell,\bs A(t)=\bs a, \rapphase^{(p)}(t)=i \right]
% 		%
% 		\\&=\int_{L(t+u),\rapphase^{(p)}(t+u)}\bs a e^{(c_i\bs S + T_{ii}\bs I)u}\bs v_{\ell,i}(x)\psi(x,i)\wrt x 
% 		%
% 		\\&\quad{}+\sum_{j\in\calS_-}\int_{L(t+u),\rapphase^{(p)}(t+u)} \int_{v=0}^u\bs a e^{(c_i\bs S + T_{ii}\bs I)v}T_{ij}e^{(c_j\bs S + T_{jj}\bs I)(u-v)}\bs v_{\ell,j}(x)\psi(x,j)\wrt v\wrt x 
% 		%
% 		\\&\quad{}+\sum_{j\in\calS_+}\int_{L(t+u),\rapphase^{(p)}(t+u)} \int_{v=0}^u\bs a e^{(c_i\bs S + T_{ii}\bs I)v}T_{ij}\bs D e^{(c_j\bs S + T_{jj}\bs I)(u-v)}\bs v_{\ell,j}(x)\psi(x,j)\wrt v\wrt x 
% 		%
% 		\\&\quad{}+\int_{L(t+u),\rapphase^{(p)}(t+u)} \int_{v=0}^u\bs a e^{(c_i\bs S + T_{ii}\bs I)v}c_i\bs s\bs \alpha e^{(c_i\bs S + T_{ii}\bs I)(u-v)}\bs v_{\ell+1,i}(x)\psi(x,i)\wrt x 
% 		%
% 		\\&\quad{}+o(u^2)
% 		%
...
% 	\begin{align*}
% 		&\lim_{u\to 0^+}u^{-1}\Bigg(\mathbb E\left[\psi( \mathcal R(L(t+u),\bs A(t+u),\rapphase^{(p)}(t+u)),\rapphase^{(p)}(t+u) )   \mid L(t)=\ell,\bs A(t)=\bs a, \rapphase^{(p)}(t)=i \right]
