...
\begin{lem}\label{lem: Dcoajc}
	...
	\end{align}
            % and 
            % \begin{align}
            % 	\left|\int_{x=0}^\Delta \widehat f^{\ell_0,(p)}_{0,-,-}(\lambda)(x,j; x_0,i)\psi(x)\wrt x - \int_{x=0}^\Delta\widehat \mu^{\ell_0}_{0,-,-}(\lambda)(x,j; x_0,i)\psi(x)\wrt x\right| \leq \left(R_{{\bs v},2}^{(p)} + \varepsilon^{(p)}\right) GF. \label{eqn: anue2}
            % \end{align} 
\end{lem}
\begin{proof} 
...
\end{align}
where \(g_1,g_2,\dots,\) are functions satisfying the Assumptions \ref{asu: g} and \({\bs v}(x)\) is a closing operator with the Properties \ref{properties: some props}. %By Lemma~\ref{cor: ksjkd}, for any \(x_0,x\in [0,\Delta)\) 
%\begin{align}
