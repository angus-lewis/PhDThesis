...
\\&\qquad{}\int_{x_n=0}^\infty \bs G_{n }(x_n)\otimes e^{\bs{S}x_n} \wrt x_n V(x) \psi(x) \wrt x \nonumber 
	...
	\end{align}
	Moreover, choosing \(\varepsilon=\var(Z)\), then, for fixed \(n\), the error terms \((|r_5(n)| + |r_6(n)| + (n-1)|r_4(n)|)\Delta F \prod_{k=2}^{n}N_{k}\) and \(\left(|r_8(n)|+|r_5(n)|+|r_6(n)| + (n-1)|r_4(n)|\right)F\Delta \prod_{k=2}^{n}N_{k}\) are \(\mathcal O(\var(Z)^{1/3})\). 
\end{lem}
\begin{proof}
	Since the proof is the same for both (\ref{eqn: rhs g 4dvfklsmv2G}) and (\ref{eqn: KAFnnmna22G}), we prove (\ref{eqn: rhs g 4dvfklsmv2G}) only. 
...
\begin{align}
	& \int_{x=0}^\Delta \int_{x_1=0}^\infty \bs G_1(x_1) \otimes \bs a^{(p)} (x_0) e^{\bs{S}^{(p)}x_1} \bs D^{(p)} (x_1) \wrt x_1
...
and 
\begin{align}
	&\int_{x\in[0,\Delta)} \int_{x_1=0}^\infty \bs G_1(x_1) \otimes \bs a^{(p)} (x_0) \bs D^{(p)} e^{\bs{S}^{(p)}x_1}\wrt x_1\bs D^{(p)} 
...
% Let \(W^{(p)}(x)\) be the closing operator such that, for \(\bs a^{(p)} \in\mathcal A^{(p)}\), \(x\in[0,\Delta)\),
% \[\bs a^{(p)} W^{(p)}(x) = \bs a^{(p)} \left(e^{\bs{S}^{(p)}x} + e^{\bs{S}^{(p)}(2\Delta-x)}\right)\left[I-e^{\bs S^{(p)} 2\Delta}\right]^{-1}\bs s^{(p)}.\]
% \begin{lem}\label{lem: akxnj2}
% 	For \(x\in[0,\Delta),u\geq 0\),  
%         \begin{align*}
%         		\bs a   e^{\bs Su}(-\bs S)^{-1} W(x) &\leq 2 \bs a e^{\bs Su} \bs e.
% 	\end{align*}
% \end{lem}
% \begin{proof}
% Let \(\bs a   \in \mathcal A\) be arbitrary. By definition 
% 	\begin{align*}
%         		\bs a  e^{\bs Su}(-\bs S)^{-1} W(x) & = \bs a  e^{\bs Su}(-\bs S)^{-1}  \left(e^{\bs{S} x} + e^{\bs{S} (2\Delta-x)}\right)\left[I-e^{\bs S 2\Delta}\right]^{-1}\bs s 
% 				\\&=\bs a  e^{\bs Su}(-\bs S)^{-1}  \left(e^{\bs{S} x} + e^{\bs{S} (2\Delta-x)}\right)\sum_{m=0}^\infty e^{\bs S 2m\Delta}\bs s 
% 				\\&=\bs a  e^{\bs Su} \left(e^{\bs{S} x} + e^{\bs{S} (2\Delta-x)}\right)\sum_{m=0}^\infty e^{\bs S 2m\Delta}\bs e 
% 	\end{align*}
% 	since \((-\bs S)^{-1}\) and \(e^{\bs Sx}\) commute and \(\bs s = -\bs S e\). 
% 	By Lemma~\ref{lem: akc} this is less than or equal to, 
% 	\begin{align}
