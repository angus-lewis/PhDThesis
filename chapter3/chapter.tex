...
\end{align*}
using a trapezoidal rule as follows. Let \(\bs f(t) = e^{\bs S t}\bs s\). Then \(\bs f(t)e^{-\lambda t}\), where \(\lambda = \Re(\lambda_i)\) is the real part of the eigenvalues of \(\bs{S}\), (they all share the same real part), is also periodic with the period \(\rho\). Hence, we can simplify the integral to a finite one;
\begin{align}
