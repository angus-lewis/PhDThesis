...
The inspiration for the approach in the chapter stems from the QBD discretisation of \cite{bo2013}, which they derive via uniformisation arguments. Recall that a QBD is a two-dimensional CTMC, \(\{(L(t),\rapphase(t))\}_{t\geq 0}\), where \(\{L(t)\}\) is the discrete level process, and \(\{\rapphase(t)\}\) is the phase process. In \cite{bo2013} the process \(\{\rapphase(t)\}\) of the fluid queue and the QBD approximation are the same, thus they are able to capture the dynamics of the phase process exactly. To approximate the level process of the fluid queue, \cite{bo2013} effectively discretise the state space of the fluid level into small intervals of width \(\Delta\). Their approximation supposes that when \(L(t)=k,\,\rapphase(t)=i\), then \(X(t)\approx k\Delta,\,\varphi(t)=i\). 
...
\end{align}
is right-continuous with left limits (c\'adl\'ag). The expression~(\ref{eqn: kfj}) is the discrete process of the fluid-queue which the level process of the QBD-RAP will approximate. 

To model the phase process of the fluid queue, \(\{\varphi(t)\}\), we will use the phase process of the QBD-RAP, \(\{\phi(t)\}\). When the phase process of the QBD-RAP is \(i\) we suppose that changes of phase occur at the constant rate \(-T_{ii}\) and, upon a change of phase, a change to phase \(j\) occurs with probability \(-T_{ij}/T_{ii}\). This choice is convenient as, when there are no boundaries for the fluid queue or QBD-RAP approximation, then the distribution of \(\{\varphi(t)\}\) and \(\{\phi(t)\}\) will be the same. 

Let \(y_k = k\Delta\), \(k=0,\dots,K+1\) and \(\calD_k = [y_k,y_{k+1}]\). We now look to model the behaviour of \(\{X(t)\}\) on an interval \(\calD_k\) over the time \([t,t+u]\) on certain events of the phase process over this time.

\subsection{Modelling the residual time to exit on no change of phase} \label{subsec:residualtime}
...
%	\\&\geq \mathbb P(Z-u\in{}(\Delta-u-\varepsilon, \Delta-u + \varepsilon))
	\\&\geq 1-\cfrac{\var(Z)}{\varepsilon^2}.
\end{align*}
Choose \(\varepsilon = \var(Z)^{1/3}\) (similar to the choice of \(\varepsilon\) in the proof of Theorem~4 of \cite{hhat2020}). Since we use Chebyshev's inequality to obtain the bound, which is general and likely to be far from sharp, we expect that the choice of \(\varepsilon\) is not important. All we require is that when \(\var(Z)\) is small, then \(\var(Z)/\varepsilon^2\) is also small. With this choice of \(\varepsilon\), then  
...
To see this, consider the event that at time \(t\) the orbit process of the QBD-RAP is \(\bs A(t) = \bs \alpha\), the phase process is \(\rapphase(t) = j\in\calS_+\), and there are no events of the QBD-RAP before time \(t+|c_i|u/|c_j|\). On this event, the orbit position at time \(t+|c_i|u/|c_j|\) will be 
...
\subsection{The residual time on a change of phase from \(i\in\calS_+\) to \(j\in\calS_-\)} \label{sec:kkkrjrjrjjrj}
...
While this does achieve what we want, it is not quite satisfactory for the purpose of our approximation scheme due to dependence on the sample path of \(\{\rapphase(t)\}\). Specifically, the evolution of the QBD-RAP from time \(t+u\) until the next event depends on the phase immediately before the change of phase at time \(t+u\), \(\rapphase(t+u^-)=i\). This increases the size of the approximating QBD-RAP as we need a separate model for each \(\rapphase(t+u^-)\in\calS_+\). Furthermore, we have not yet considered how to model any further changes from \(\calS_-\) to \(\calS_+\) or beyond, which further complicates matters. 
...
To approximate the fluid queue, we suppose that the initial orbit position is \(\bs A(0)=\bs \alpha\), the initial phase is \(\rapphase(0)=j_0\) and that \(\varphi(t)=\rapphase(t),\,t\in[0,t_3]\). The sample path of the orbit process on \(t\in{}[0,t_3)\), is 
\begin{align*}
\bs A(t) &= \begin{cases} 
...
\paragraph{The dynamics of the orbit process and phase} On \({\phi}(t)=i\), we suppose that between event epochs the process \(\{\bs A(t)\}\) evolves deterministically according to the differential equation 
\begin{align}\label{eqn: At DE}
\cfrac{\wrt}{\wrt t}\bs A(t) = \bs A(t)(\bs{S}_i+T_{ii}\bs I - \bs A(t) (\bs{S}_i + T_{ii}\bs{I}_p)\bs e\cdot\bs{I}_p).
\end{align}
The ODE (\ref{eqn: At DE}) describes the movement of the orbit process on the event that the phase and level are constant. This implies that the diagonal blocks of the generator of the QBD-RAP will be the matrices \(\bs S_i-T_{ii}\). 

We now argue that the ODE (\ref{eqn: At DE}) is a reasonable model given our discussions about using the residual time to model the distance from the cell edges and given our assumptions on the phase dynamics.

Let \(\bs a \in{}\mathcal A\) be arbitrary. On the events that no jumps or changes of phase occur between time \(u\) and \(u+v\), \(\bs A(u) = \bs a\in\mathcal A\) and \({\phi}(u)=i\), the solution to (\ref{eqn: At DE}) states that \(\bs A({u+v})\) evolves deterministically according to 
\[\bs A({u+v}) =  \cfrac{\bs a e^{(\bs{S}_i+T_{ii}\bs{I}_p)v}}{\bs a e^{(\bs{S}_i+T_{ii}\bs{I}_p)v} \bs e} = \cfrac{\bs a e^{\bs{S}_i v}}{\bs a e^{\bs{S}_i v} \bs e}.\]
Thus, when \(\bs a=\bs\alpha\) the orbit position models the distance from the cell edges as discussed in Section~\ref{sec: modelling}, as required.

At time \(t\) an event occurs at rate 
\[\bs A(t) (\bs{S}_i - T_{ii}\bs{I}_p)\bs e = \bs A(t)\bs s_i - T_{ii}.\]
More precisely, an event corresponding to a change in phase for \({\phi}(t)\) occurs at rate \[-\bs A(t) T_{ii}\bs e = -T_{ii}\] 
which is constant as required, and an event corresponding to a \emph{change of level} occurs at rate \[-\bs A(t) \bs{S}_i\bs e = \bs A(t) \bs s_i.\] Later, we will make clear why we say that the latter event corresponds to a change of level. Upon an event occurring at time \(t\), with probability \({-T_{ii}}/{\left(-T_{ii} + \bs A({t^-})\bs s_i\right)}\) the event corresponds to a change of phase and with probability \({\bs A({t^-})\bs s_i}/{\left(-T_{ii} + \bs A({t^-})\bs s_i\right)}\) the event corresponds a change of level. At time \(t\), the distribution of time until the next event is the minimum of the time until a change of phase and a matrix exponential random variable \(ME(\bs A(t),\bs S_i)\). 

Upon an event corresponding to a change of level occurring at time \(t\) we suppose that the process \(\{\bs A(t)\}\) jumps to \(\bs A(t) = \bs \alpha\) and that the phase remains constant. Note that a change of level may only occur if the phase is in \(\calS_+\) or \(\calS_-\).

Upon an event corresponding to a change of phase from \(i\) to \(j\neq i\) occurring at time \(t\), there are two possibilities; either \(\mathrm{sign}(c_i)=\mathrm{sign}(c_j)\), or \(\mathrm{sign}(c_i)\neq \mathrm{sign}(c_j)\). As discussed earlier, for states \(i\in\mathcal S_0\) we must specify some association with either \(\mathcal S_+\) or \(\mathcal S_-\). Here we choose the augmented state space approach and duplicate \(\calS_0\) and associate one copy with \(i\in\calS_+\) and one copy with \(\calS_-\); call these \(\calS_{+0}\) and \(\calS_{-0}\), respectively. We take \(\mathrm{sign}(c_i)=+\) for \(i\in\calS_{+0}\) and \(\mathrm{sign}(c_i)=-\) for \(i\in\calS_{-0}\). In the case
\begin{enumerate}
	\item \(\mathrm{sign}(c_i)=\mathrm{sign}(c_j)\): at the time, \(t\), of the event, \(\{\bs A(t)\}\) is unchanged but immediately begins to evolve according to (\ref{eqn: At DE}) with \(i\) replaced by \(j\), while \(\left\{{\phi}(t)\right\}\) jumps from \({\phi}(t^-)=i\) to \({\phi}(t)=j\). Since the orbit is unchanged upon these events, the matrix which maps the position of the orbit before the jump to the position of the orbit after the jump is the identity, \(\bs I\). Thus, the diagonal blocks of the generator of the QBD-RAP which describe its motion among the phases \(\calS_{m}\cup\calS_{m0},\,m\in\{+,-\}\) are 
	\[\left[\begin{array}{cc}
		\bs C_m \otimes \bs S + \bs T_{mm}\otimes \bs I & \bs T_{m0}\otimes \bs I \\
		\bs T_{0m}\otimes \bs I & \bs T_{00}\otimes \bs I
	\end{array}\right].\]
	The matrix \(\bs C_m\otimes \bs S\) is block diagonal with blocks \(|c_i|\bs S=\bs S_i\) and \(\bs T_{mn}\otimes \bs I\) are block matrices with blocks \(T_{ij}\bs I\).  
	\item \(\mathrm{sign}(c_i)\neq \mathrm{sign}(c_j)\): at the time, \(t\), of the event, \(\{\bs A(t)\}\) the orbit jumps from \(\bs A(t^-)\) to 
	\[\bs A(t) = \cfrac{\bs A({t^-}) T_{ij}\bs{D}}{\bs A({t^-}) T_{ij} \bs{D} \bs e} = \cfrac{\bs A({t^-}) T_{ij}\bs{D}}{T_{ij} } = \bs A({t^-}) \bs{D}, \]
	assuming \(\bs D\bs e=\bs e\), and then immediately proceeds to evolve according to (\ref{eqn: At DE}) with \(i\) replaced by \(j\), and \(\left\{{\phi}(t)\right\}\) jumps from \({\phi}(t^-)=i\) to \({\phi}(t)=j\). Since we are assuming the augmented state space model, then jumps out of \(\calS_m\cup\calS_{m0}\) can only jump to phases \(j\in\calS_n,\,m,n\in\{+,-\},\,m\neq n\), by construction. The off-diagonal block matrices in the generator of the QBD-RAP which describe the movement of the process while the level remains constant, but the phase jumps from \(\calS_m\cup\calS_{m0}\) to \(\calS_n\cup \calS_{n0},\,m,n\in\{+,-\},\, m\neq n,\) are the matrices 
	\[\left[\begin{array}{cc}
		\bs T_{mn}\otimes \bs D & \bs 0 \\
		\bs T_{0n}\otimes \bs D & \bs 0 
	\end{array}\right].\]
\end{enumerate}

%#The two scenarios, \(\mathrm{sign}(c_i)\neq \mathrm{sign}(c_j)\) and \(\mathrm{sign}(c_i)= \mathrm{sign}(c_j)\), can be written succinctly by stating that, at the time of an event corresponding to a change of phase from \(i\) to \(j\), \(\{\bs A(t)\}\) jumps to \(\bs A(t)\bs{D}^{1(\mathrm{sign}(c_i)\neq \mathrm{sign}(c_j))}\) and begins to evolve according to (\ref{eqn: At DE}) with \(i\) replaced by \(j\), meanwhile \(\left\{{\phi}(t)\right\}\) jumps from \(\phi(t^-)=i\) to \(\phi(t)=j\). % Moreover, given a change of phase event occurs at time \(t\) with \(\phi(t^-)=i\) then, with probability \(\cfrac{\bs A(t^-)T_{ij}\bs{D}^{1(\mathrm{sign}(c_i)\neq \mathrm{sign}(c_j))}\bs e}{-\bs A(t^{-})T_{ii}\bs e} = \cfrac{T_{ij}}{-T_{ii}}\), the event corresponds to a change of phase from \(i\) to \(j\). 
...
To event epochs of \(\left\{(\bs A(t),{\rapphase}(t))\right\}_{t\geq0}\) we associate marks \(\left\{-1,0,+1\right\}\) in the following way.
\begin{itemize}
	\item To event epochs corresponding to a change of phase of \( {\rapphase}(t)\) we associate the mark \(0\).
	\item To event epochs at time \(t\) which correspond to a change in level and for which \( {\rapphase}(t^-)=i\in\mathcal S_-\) we associate the mark \(-1\).
	\item To event epochs at time \(t\) which correspond to a change in level and for which \( {\rapphase}(t^-)=i\in\mathcal S_+\) we associate the mark \(+1\).
\end{itemize}
Now define \(N_+(t)\) (\(N_-(t)\)) as the simple point process which counts the number of event epochs with marks \(+1\) (\(-1\)) which have occurred in the time up to and including time \(t\). The level process of the QBD-RAP is given by \( L(t) = N_+(t)-N_-(t)\) and is skip-free, by construction. The process \(\left\{(L(t),\bs A(t),\rapphase(t))\right\}_{t\geq 0}\) forms a QBD with RAP components.

Since the phase remains constant on a change of level and the instantaneous rate at which a change of level occurs at time \(t\) in phase \(i\) is \(\bs A(t)\bs s_i\), then the block matrix of the generator which describes the rate a which a change of level occurs when \(\phi(t)\in\calS_+\) is 
\begin{align*}
	\left[\begin{array}{cccc}
		\bs C_+ \otimes (\bs s\bs\alpha) & & & \\
		& \bs 0 & & \\ 
		& & \bs 0 & \\ 
		& & & \bs 0 
	\end{array}
...
\begin{align*}
	...
	\right],
\end{align*}
where \(\bs C_m\otimes (\bs s\bs\alpha)\), \(m\in\{+,-\}\), is block-diagonal with blocks \(|c_i|\bs s \bs\alpha\). 

\paragraph{The generator of the QBD-RAP}
Assuming we have used the augmented state-space representation of the fluid queue described in Section~\ref{sec: zero states} and that there are no boundaries, the generator of the approximating QBD-RAP \(\left\{(L(t),\bs A(t),{\rapphase}(t))\right\}\), is 
\begin{align}\label{eqn: BBBB}
...
{where,}
\begin{align}
	\bs B_0 &= \left[\begin{array}{ccc} \bs B_{++} & \bs B_{+-} \\ \bs B_{-+} & \bs B_{--}\end{array}\right]\label{eqn: BBBB2}
\end{align}
\begin{align}
	...
	&& \bs{B}_{+1} = \left[\begin{array}{cccc}
		...
		& & \bs 0 & \\ 
		& & & \bs 0
		\end{array}\right],\label{eqn: BBBB2}
\end{align}
and
\begin{align}
	...
	\\ \bs{B}_{-+}  &=  \left[\begin{array}{cc} \bs{T}_{-+}\otimes \bs{D}  & \bs 0 \\ \bs T_{0+}\otimes \bs D  & \bs 0 \end{array}\right] ,
	% 
	\bs{B}_{--}  = \left[\begin{array}{cc} \bs{C}_-\otimes \bs{S}  + \bs{T}_{--}\otimes \bs{I} & \bs T_{-0}\otimes \bs I \\ \bs T_{0-}\otimes \bs I & \bs T_{00}\otimes \bs I\end{array}\right].\label{eqn: BBBB3}
\end{align}
The tridiagonal structure is a generator arises from the skip-free nature of the level process and the sub-blocks and parameters are as described above.

% The lower diagonal matrices \(\bs B_{-1}\) contain the rates at which the QBD-RAP transitions from level \(k\) to level \(k-1\), \(k\in\mathbb Z\). As it is only possible to transition from level \(k\) to \(k-1\) in phases with a negative rate and because the phase remains the same upon a change of level, the only non-zero entries are in \(\bs B_{-1}\) are in the sub-block corresponding to \(\calS_-\). As we are using the augmented states space model and the phases are assumed to be ordered by \(\calS_+,\,\calS_{+0},\,\calS_-,\,\calS_{0-}\), so the sub-block corresponding to \(\calS_-\) is the third diagonal block. 

% Similarly, the upper diagonal matrices \(\bs B_{+1}\) contain the rates at which the QBD-RAP transitions from level \(k\) to level \(k+1\), \(k\in\mathbb Z\). As it is only possible to transition from level \(k\) to \(k+1\) in phases with a positive rate and because the phase remains the same upon a change of level, the only non-zero entries are in \(\bs B_{+1}\) are in the block corresponding to \(\calS_+\). 

% The diagonal blocks \(\bs B_0\) describe the orbit and phase dynamics within level \(k\), \(k\in\mathbb Z\). The sub-block \(\bs B_{+-}\) (\(\bs B_{-+}\)) describes the transitions between phases in \(\calS_+\cup\calS_{+0}\) (\(\calS_-\cup\calS_{-0}\)) to phases in \(\calS_-\cup\calS_{-0}\) (\(\calS_+\cup\calS_{+0}\)). Ultimately, for these transitions the phase goes from the states associated with \(\calS_+\) (\(\calS_-\)) to the set of states associated with \(\calS_-\) (\(\calS_+\)) so upon these transitions the orbit jumps according to \(\bs D\), hence \(\bs D\) appears in these blocks. Further, on such transitions, the phase always ends up in \(\calS_-\) (\(\calS_+\)) by construction of the augmented phase process. Hence, the first block-column corresponding to phases \(\calS_-\) (\(\calS_+\)) of these matrices is non-zero and the second block-column corresponding to phases \(\calS_{-0}\) (\(\calS_{+0}\)) is zero.

% The sub-block \(\bs B_{++}\) (\(\bs B_{--}\)) describes the evolution of the QBD-RAP while it remains in phases \(\calS_+\cup\calS_{+0}\) (\(\calS_-\cup\calS_{-0}\)). Notice that diagonal entries for phases with a zero rate have no \(\bs S\) component and therefore, when the process is in these phases, the orbit is constant. 
...
\end{thm}
\begin{proof}
	Let \(\{\tau_n\}_{n\geq 0}\) with \(\tau_0=0\) and \(\tau_n\) the time of the \(n\)-th change of level of the QBD-RAP. Consider partitioning \(\left\{\Theta_i> t \right\}\) with respect to \(\left\{\tau_{n-1}< t \leq\tau_n\right\},\,n=1,2,\dots\). For \(n=1\), by (\ref{eqn: rapproperty}) and Theorem~\ref{thm:qbdrapchar}, we can write
	\begin{align*}
	...
	\end{align*}
	For \(n>1\), again using (\ref{eqn: rapproperty}) and Theorem~\ref{thm:qbdrapchar}, by partitioning on the times of the first \(n-1\) level changes, \(\tau_1,\dots,\tau_n\), we get 
	\begin{align}
...
%where each \(\bs v_{k,i}\) is such that \((\bs v_{k,i}, \bs{S}_i,\bs s_i)\) is a valid representation of an ME. The initial distribution of the phase and level is given by the product \(\bs v_{k,i}\bs e = \mathbb P( L(0)=k, \rapphase(0)=i)\) and, given \(\left\{ L(0)=k, \rapphase(0)=i\right\}\), the initial position of \(\left\{\bs A(t)\right\}\) is \(\cfrac{\bs v_{k,i}}{\bs v_{k,i}\bs e} \). 
%
%The product  
...
%gives the distribution at time \(t\): for \(k\in\mathbb Z\),  \(\mathbb P({L}(t) = k, \rapphase(t)=i) = \bs v_{k,1}(t)\bs e\) and, given \(\left\{{L}(t) = k, \rapphase(t)=i\right\}\), the position of \(\left\{\bs A(t)\right\}\) is \(\bs A(t) =\cfrac{\bs v_{k,i}(t)}{\bs v_{k,i}(t)\bs e}. \)
...
Suppose that, upon hitting the boundary in phase \(i\in\calS_-\), the phase jumps from \(i\) to \(j\in\calS\) with probability \(\widecheck p_{ij} \). If \(j\in\calS_-\cup\calS_0\) the process remains at the boundary and the phase process evolves among states in \(\calS_-\cup\calS_0\) until the first transition to a state in \(\calS_+\), at which point the level \(\{X(t)\}\) immediately leaves the boundary -- we will call this a \emph{sticky} boundary. If \(j\in\calS_+\) the process immediately leaves the boundary -- we will call this a reflecting boundary. 
We collect the probabilities \(\widecheck p_{ij} \) into the matrices \(\widecheck{\bs P}_{-+} = [\widecheck p_{ij} ]_{i\in\calS_-,j\in\calS_+},\) \(\widecheck{\bs P}_{-0}  = [\widecheck p_{ij} ]_{i\in\calS_-,j\in\calS_0},\) and \(\widecheck{\bs P}_{--}  = [\widecheck p_{ij} ]_{i\in\calS_-,j\in\calS_-}.\)
...
For a sticky lower boundary at \(0\), given the fluid level reaches zero at time \(u\) in phase \(i\in\mathcal S_-\), then the phase transitions to some phase \(j\in\calS_-\cup\calS_0\) with probability \(\widecheck p_{ij} \), and the distribution of the phase \(\rapphase(t)\) (on the event that the process has not left the boundary at or before time \(t>u\)) is given by the elements of the vector
\[\widecheck p_{ij}  \bs e_j \exp\left({\left[\begin{array}{cc} \bs T_{--} & \bs T_{-0} \\ \bs T_{0-} & \bs T_{00}\end{array}\right]}(t-u)\right).\] 
The distribution for the time until \(\{X(t)\}\) leaves \(\{0\}\) for the first time after \(u\) is 
\[1-\widecheck p_{ij} \bs e_j \exp\left({\left[\begin{array}{cc} \bs T_{--} & \bs T_{-0} \\ \bs T_{0-} & \bs T_{00}\end{array}\right]}(t-u)\right)\bs e.\] 
...
\[\widecheck p_{ij} \bs e_j \exp\left({\left[\begin{array}{cc} \bs T_{--} & \bs T_{-0} \\ \bs T_{0-} & \bs T_{00}\end{array}\right]}(t-u)\right)\left[\begin{array}{c} \bs T_{-+} \\ \bs T_{0+}\end{array}\right]\bs e_k.\] 
For a reflecting boundary, given the boundary is hit in phase \(i\in\calS_-\), the phase upon leaving the boundary is \(j\in\calS_+\) with probability \(\widecheck p_{ij} \). Upon leaving the boundary, the appropriate orbit position for the QBD-RAP is \(\bs \alpha\), as this corresponds to the fluid level \(0\). 
...
\[ c_i\widecheck p_{ij}  \bs A(t)\bs s.\]
...
&\left[\begin{array}{cc|cccc|cc}
	...
	\bs 0 & \bs 0 &\multicolumn{4}{c|}{\multirow{4}{*}{$\widecheck{\bs B}_{0}$}} & \multirow{4}{*}{$\bs B_{+1}$}& \\
	\bs 0 & \bs 0 & \multicolumn{4}{c|}{} & &\\
	(\bs C_-\widecheck{\bs P}_{--} )\otimes \bs s &(\bs C_-\widecheck{\bs P}_{-0}) \otimes \bs s & \multicolumn{4}{c|}{} &&\\
	\bs 0 & \bs 0 & \multicolumn{4}{c|}{} && \ddots\\ \hline
	& & \multicolumn{4}{c|}{\bs B_{-1}} & \bs B_0 &\\
	& & \multicolumn{3}{c}{}& \ddots &  & \ddots
	\end{array}\right],\label{eqn: BBBBbndry}
\end{align}
where 
\begin{align}
\widecheck{\bs B}_{0} = \left[\begin{array}{cccc}
	 ...
	 (\bs C_-\widecheck{\bs P}_{-+})\otimes \bs s \bs \alpha + \bs T_{-+}\otimes \bs D  &  & \bs{C}_-\otimes \bs{S} + \bs{T}_{--}\otimes \bs{I} & \bs{T}_{-0}\otimes \bs{I} \\
	 \bs{T}_{0+}\otimes \bs{D} & & \bs{T}_{0-}\otimes \bs{I} & \bs{T}_{00}\otimes \bs{I} 
	\end{array}\right].\label{eqn: BBBBbndry2}
\end{align}
The top-left block of the generator contains the rates at which probability moves between the point masses at the sticky boundary. The block below that in the generator contains the approximation to the rates that the density of the fluid queue transitions into point masses at the sticky boundary. This can only occur from phases in \(\calS_-\), which is the third sub-block, hence only the third row of block matrices is non-zero. The block to the right of the top-left block of the generator contains the rates that point masses transition into density as the fluid process leaves the boundary. These transitions only occur to phases in \(\calS_+\) so only the first sub-block-column of this block is non-zero. The term \((\bs C_-\widecheck{\bs P}_{-+})\otimes \bs s \bs \alpha\) which has been added in \(\widecheck{\bs B}_{0}\) incorporates the reflecting boundary behaviour. 

%The top-left block contains the rate the probability moves between the point masses at the sticky boundary. The block below that contains the approximation to the rates that the density of the fluid queue transitions into point masses at the sticky boundary. This can only occur from phases in \(\calS_-\), which is the third sub-block, hence only the third row of block matrices is non-zero. The block to the right of the top-left block contains the rates that point masses transition into density as the fluid process leaves the boundary. These transitions occur at rates stored in the matrices \(\bs T_{-+}\) and \(\bs T_{0+}\) and upon leaving the boundary, the orbit is \(\bs \alpha\). The term \((\bs C_-\widecheck{\bs P}_{-+})\otimes \bs s \bs \alpha\) which has been added in \(\widecheck{\bs B}_{0}\) incorporates the reflecting boundary behaviour. The process is reflected from phase \(i\in\calS_-\) to a phase \(j\in\calS_+\) with probability \([\widecheck{\bs P_{-+}}]_{ij}\) and upon reflection the orbit process is set to \(\bs \alpha\).

Upper boundaries can be included in an analogous manner ad we denote the transition probabilities upon hitting a boundary by \(\widehat p_{ij} \) and define the matrices \(\widehat{\bs P}_{+-} = [\widehat p_{ij}]_{i\in\calS_+,j\in\calS_{-}},\) \(\widehat{\bs P}_{+0} = [\widehat p_{ij}]_{i\in\calS_+,j\in\calS_0},\) and \(\widehat{\bs P}_{++}  = [\widehat p_{ij}]_{i\in\calS_+,j\in\calS_+}.\) 
...
%To this point, we have assumed that \((X(0),\rapphase(0))=(y_\ell,i), \,i\in\mathcal S_+\) or \((X(0),\rapphase(0))=(y_{\ell+1},i), \,i\in\mathcal S_-\), i.e., the initial position of the fluid is at the edge of the cell. In these cases, the appropriate initial orbit position is \(\bs \alpha\). We now generalise this so that we can approximate arbitrary initial conditions. 

%To motivate the construction, first consider the initial condition for the QBD-RAP approximation, \((L(0),\bs A(0), \rapphase(0))=(\ell,\bs \alpha,i)\), corresponding to the initial condition of the fluid queue \((X(0),\rapphase(0))=(y_\ell,i),\) for some \(i\in\mathcal S_+\). After time \(t\), \(c_it\in{}[0,\Delta)\), and assuming that no events of the QBD-RAP have occurred by this time, the position of the QBD-RAP is \((L(t),\bs A(t),\rapphase(t)) = (\ell, \bs A(t), i),\) where \(\bs A(t) = \cfrac{\bs \alpha e^{c_i\bs{S}t}}{\bs \alpha e^{c_i\bs{S}t}\bs e}\). The level of the fluid at the same time is \(X(t) = y_\ell + c_i t\). 
We argued earlier that we can think of the orbit \(\bs k(x)=\bs \alpha e^{\bs S x}/(\bs \alpha e^{\bs S x}\bs e)\) as corresponding to the fluid being a distance of \(x\) from the left boundary of an interval when \(i\in\calS_+\), or from the right boundary of an interval when \(i\in\calS_-\). With this interpretation, an approximation to the initial condition \(\bs X(0) = (X(0),\varphi(0))=(x_0,i),\,x_0\in\mathcal D_{\ell,i},\,i\in\mathcal S_+\cup\calS_{+0}\) is to set the initial orbit to \(\bs k(x_0-y_{\ell})\) and the initial phase to \(\rapphase(0)=i\). Similarly, an approximation to the initial condition \(\bs X(0)=(x_0,i), \,x_0\in\mathcal D_{\ell,i}, x_0\neq y_{\ell+1},\,i\in\mathcal S_-\cup\calS_{-0}\) is to set the initial orbit position to \(\bs k(y_{\ell+1}-x_0)\) and the initial phase to \(\rapphase(0)=i\). 
...
While the augmented state space scheme described above is convergent, the analysis of the QBD-RAP scheme is \emph{greatly} simplified if we only allow initial conditions which allocate zero probability to phases in \(i\in\calS_{+0}\cup\calS_{-0}\). To see why initial mass in \(\mathcal S_{+0}\cup\calS_{-0}\) might introduce further complexity, consider a sample path with \(\rapphase(0)=k\in\calS_{+0}\) and \(\bs A(0)=\bs a\in\mathcal A\). On \(\rapphase(u)\in\calS_{+0},\) for \(u\in{}[0,t)\), then \(\bs A(u) = \bs a \) is constant. Upon the phase leaving \(\calS_{+0}\), at time \(t\), say, the phase will either jump to \(\calS_+\) or \(\calS_-\). If \(\rapphase(t)\) jumps to \(i\in\calS_+\), the orbit process at time \(t\) will be \(\bs A(t)=\bs a \) and the process evolve from time \(t\) as if it has started in phase \(i\in\calS_+\) with initial orbit \(\bs a \) -- there are no major issues in this case. If, however, \(\rapphase(t)\) jumps to \(i\in\calS_-\), then the orbit process will jump to \(\bs A(t) = \bs a \bs D\) at time \(t\). From time \(t\) onward the QBD-RAP process will evolve as if it has started in phase \(i\in\calS_-\) with initial orbit \(\bs a \bs D\). With the way we have structured the proof of convergence, we have to complete two analyses of the QBD-RAP process -- one for the initial condition \(\bs a\) and another for the initial condition \(\bs a\bs D\). 

A solution to this problem is to augment a set of ephemeral states to the QBD-RAP which capture the initial sojourn in \(\calS_0\) only. Suppose we wish to approximate a fluid queue with the initial condition \(\mu_k\), \(k\in\calS_0\), where \(\mu_k\) has mass 1. Let us denote the set of ephemeral states by \(\calS_0^{*,k}\) and each state in \(\calS_0^{*,k}\) by \(i^*\). Each state \(i^*\in\calS_0^{*,k}\) is a copy of \(i\in\calS_0\), and, in \(\calS_0^{*,k}\) the phase process has the same phase dynamics as it does in \(\calS_0\). At time \(0\), we start the QBD-RAP in the ephemeral set of states \(\calS_0^{*,k}\) in phase \(k^*\in\calS_0^{*,k}\). While in \(\calS_0^{*,k}\), the phase process evolves between phases according to the generator \(\bs T_{00}\) and the orbit remains constant. Upon exiting the ephemeral set \(\calS_0^{*,k}\) at time \(t\), given the phase at \(t^-\) is \(j^*\in\calS_0^{*,k}\), the QBD-RAP process jumps to \(L(t)=\ell,\, \bs A(t)=\int_{\calD_{\ell,i}}\bs a_{\ell,i}(x)\wrt \mu_k,\,\rapphase(t)=i\), where \(\ell\in\mathcal K\), \(i\in\calS_+\cup\calS_-\) with probability 
\[\int_{\calD_{\ell,i}}\bs a_{\ell,i}(x)\wrt \mu_k\bs e\cfrac{T_{j^*i}}{-T_{j^*j^*}}.\] 

The level and orbit process are constant during the sojourn in \(\calS_0^{*,k}\) as all information about the level and orbit during this time is contained within the initial condition \(\mu_k\), and the only thing that changes is the phase. 

To include the ephemeral states within the generator of the QBD-RAP we can write, 
\begin{align*}
	\bs B &=\left[\begin{array}{c|ccccc}
		\bs T_{00} & \bs B_{-1}^{*,k} &\bs B_{0}^{*,k}&\bs B_{1}^{*,k} &\bs B_{2}^{*,k}& \hdots \\\hline
		&\widecheck{\bs B}_{0} &  \widecheck{\bs B}_{+1} &&&\\
		& \widecheck{\bs B}_{-1}&{\bs B}_{0}&\bs B_{+1}&& \\
		&&\bs B_{-1} & \bs B_0&B_{+1}& \\
		&&&\ddots&\ddots & \ddots
	\end{array}\right],
\end{align*}
where 
\begin{align*}
	\bs B_{-1}^{*,k} &= \vligne{\bs T_{0-}\mu_k(\{0\}) & \bs 0} & \bs B_{\ell}^{*,k} &= \vligne{\bs T_{0+}\otimes \bs a_\ell^+ & \bs 0 & \bs T_{0-}\otimes \bs a_\ell^- & \bs 0},\, \ell=0,1,2,...,
	%
	% \\\widecheck{\bs B}_{0} &= \left[\begin{array}{cc}\bs T_{--} & \bs T_{-0}\\\bs T_{0-} & \bs T_{00}\end{array}\right], 
	% & \widecheck{\bs B}_{+1} &= \left[\begin{array}{cccc}\bs T_{-+}\otimes \bs \alpha & \bs 0 & \bs 0 & \bs 0 \\ \bs T_{0+}\otimes \bs \alpha & \bs 0 & \bs 0 & \bs 0 \end{array}\right],
	% %
	% \\\widecheck{\bs B}_{-1} &= \left[\begin{array}{cc} \bs0&\bs0\\\bs0&\bs0\\(\bs C_-\bs P_{--})\otimes \bs s & (\bs C_-\bs P_{-0})\otimes \bs s \\\bs0&\bs0\end{array}\right], &&
\end{align*}
and 
\begin{align*}
	\bs a_\ell^r = \int_{\calD_{\ell,r}} \bs a_{\ell,r}(x)\wrt \mu_k,\, \ell\in\mathcal K^\circ,\, r\in\{+,-\}. 
\end{align*}
The blocks \(\widecheck{\bs B}_{-1}\), \(\widecheck{\bs B}_{0}\), \(\widecheck{\bs B}_{+1}\), \({\bs B_{-1}}\), \({\bs B_{0}}\), \({\bs B_{+1}}\), are as before. In the top-left block of \(\bs B\) is the block \(\bs T_{00}\) which contains the rates at which the process moves between phases in \(\calS_0^{*,k}\). The block \(\bs B_{\ell}^{*,k}\) contains the rates that the phase changes from \(\calS_0^{*,k}\) to \(\calS_+\) or \(\calS_-\) and also contains the information about where the orbit position is after these transitions. 

Since \(\bs a_{\ell,i}(x)\in\mathcal A\) for all \(x\geq 0\) and \(\mu_k\) is a probability measure, then \(\bs a_\ell^r=\int_{\calD_{\ell,i}}\bs a_{\ell,i}(x)\wrt \mu_k\in \mathcal A\) is a convex combination of vectors in \(\mathcal A\) and, since \(\mathcal A\) is convex, so \(\bs a_\ell^r\in\mathcal A\). Therefore, the orbit after the transition out of the ephemeral set \(\calS_{0}^{*,k}\) is in \(\mathcal A\) and therefore the QBD-RAP with the additional ephemeral states is a valid QBD-RAP. 
...
\section{Recovering intra-level approximations to the distribution of \(X(t)\) using \(\bs A(t)\) -- some intuitive ideas}\label{sec: closing}
...
\end{align}
This may be of interest in its own right. However, for some applications, this approximation may be too coarse -- the level process tells us nothing about where in the intervals \(\calD_{k,i}\) the fluid level might be. The purpose of this section is to introduce the idea of \emph{closing operators} \(\bs v(x)\) which are used to extract intra-level approximations to the distribution of \(X(t)\) using \(\bs A(t)\) and also to describe some intuition about how we might construct \(\bs v(x)\). The intra-level approximation obtained using \(\bs A(t)\) and \(\bs v(x)\) is entirely post-hoc in the sense that it does not affect the dynamics of the QBD-RAP itself, we only take the information contained in \(\mathbb E[\bs A(t)1(L(t)=\ell,\rapphase(t)=i)]\) and rewrite it in a descriptive way. 

Suppose that the QBD-RAP is in level \(L(t)=\ell\), phase \(\phi(t)=j\in\calS_+\cup\calS_{+0}\), and the orbit is \(\bs A(t)=\bs a\). If the QBD-RAP remains in phase \(j\), the QBD-RAP approximation will transition out of level \(\ell\) in the infinitesimal time interval \([t+u,t+u+\wrt u]\) with probability \(\bs a e^{|c_j|\bs{S}u}|c_j|\bs s\wrt u\). At the time of the change of level we estimate the position of \(X(t+u)\) by \(X(t+u)\approx y_{\ell+1}\). Tracing this back to time \(t\), we estimate the position of \(X(t)\) as \(X(t)\approx y_{\ell+1} - |c_j|u\). Reverse engineering this logic, the approximation to the probability of \(X(t) \in\wrt x\) is \(\bs a e^{|c_j|\bs S(y_{\ell+1}-x)/|c_j|}|c_j|\bs s \wrt x/|c_j|\), since \(\wrt x = |c_j|\wrt u\) where \(\wrt x\) is an infinitesimal in space and \(\wrt u\) is an infinitesimal with respect to time.

Similarly, for \(j\in\mathcal S_-\cup\calS_{-0}\), if the QBD-RAP remains in phase \(j\) then the QBD-RAP approximation will transition out of level \(\ell\) in the infinitesimal time interval \([t+u,t+u+\wrt u]\) with probability \(\bs a e^{|c_j|\bs{S}u}|c_j|\bs s\wrt u\). At the time of the transition of level, we estimate the position of \(X(t+u)\) by \(X(t+u)\approx y_{\ell}\). Tracing this back to time \(t\), we estimate the position of \(X(t)\) as \(X(t)\approx y_{\ell} + |c_j|u\). Reverse engineering this logic, the approximation to the probability \(X(t) \in\wrt x\) is \(\bs a e^{|c_j|\bs S(x-y_{\ell})/|c_j|}|c_j|\bs s \wrt x/|c_j|\), since \(\wrt x = |c_j|\wrt u\). 
...
\begin{align}
		...
		\bs a e^{\bs{S}z_{\ell,j}(x)} \bs s \wrt x,
		%
...
\end{align}
where \(E\subseteq\mathcal D_{\ell,j}\), \(x_0\in\mathcal D_{\ell_0,i}\), \(\bs y_0 = (\ell_0,\bs a_{\ell_0,i}(x_0),i)\), and 
\[
	z_{\ell,j}(x) = \begin{cases} 
		x - y_\ell & j\in\calS_-, \\
		y_{\ell+1} - x & j\in\calS_+.
	\end{cases}
\]
%For example, for the augmented scheme in Section~\ref{subsec: augmented }, we approximate \(\mathbb P(X(t)\in{}\wrt x , \rapphase(t) = j\mid X(0)=x_0, \rapphase(0) = i)\) by 
%\begin{align}
%	\label{eqn: density approx aug}
%		&\int_{\bs a \in\mathcal A}\mathbb P(L(t) = \ell, \bs A(t)\in\wrt \bs a,\rapphase(t) = j\mid L(0)=\ell_0, \bs A(0) = \bs a, \rapphase(0) = i) \bs a e^{\bs{S}(y_{\ell+1}-x)} \bs s \wrt x & j\in\mathcal S_+\cup \mathcal S_{+0},
%		\\\label{eqn: density approx aug2}
%		&\int_{\bs a \in\mathcal A}\mathbb P(L(t) = \ell, \bs A(t)\in\wrt \bs a,\rapphase(t) = j\mid L(0)=\ell_0, \bs A(0) = \bs a, \rapphase(0) = i) \bs a e^{\bs{S}(x-y_\ell)} \bs s \wrt x & j\in\mathcal S_-\cup \mathcal S_{-0},
%\end{align}
...
\begin{align*}
	&\sum_{\ell\in\mathcal K}\sum_{j\in\mathcal S}\int_{x\in\mathcal D_{\ell,j}}\displaystyle\int_{\bs a \in\mathcal A}\mathbb P(\bs Y(t) \in{}(\ell, \wrt \bs a, j)\mid \bs Y(0)=\bs y_0) \bs a e^{ \bs{S}z_{\ell,j}(x)} \bs s \wrt x
	...
	% \\&<1,
\end{align*}
since \(\bs a e^{\bs{S}u} \bs e>0\) for any \(u>0\).
% For an orbit position \(\bs A(t)=\bs a\), the amount of mass missing from each level and phase is 
% \[\mathbb P(\bs Y(t)\in{}(\ell, \wrt \bs a,j)\mid \bs Y(0)=\bs y_0) \bs a e^{\bs{S}\Delta} \bs e.\]
...
%One way to rectify this is to renormalise the estimates in the right-hand side of (\ref{eqn: density approx aug})-(\ref{eqn: density approx aug2}) with the missing mass, i.e.~the approximation to \(\mathbb P(X(t)\in{}\wrt x , \rapphase(t) = j\mid X(0)=x_0, \rapphase(0) = i) \) is
%\begin{align}
...
\begin{align}
%	&\mathbb P(X(t)\in{}\wrt x  , \rapphase(t) = j\mid X(0)=x_0, \rapphase(0) = i) \nonumber
...
which is less than the quantity \(\bs a e^{\bs S \Delta}\bs e\) from the approximation scheme (\ref{eqn: density approx aug}). The choice of truncating at \(2\Delta\) in this construction is convenient as in naturally leads to a density on \(x\in[0,\Delta)\).  
...
In this section we have established some intuitive ideas about how to extract an approximation to the density of \(X(t)\) from the orbit \(\bs A(t)\) using the idea of a closing operator, \(\bs v(x)\). As we shall see, given certain properties of \({\bs v}(x)\) we can prove that the approximation scheme converges, and ensures positivity. In the cases above, all the closing operators (\ref{eqn: alal})-(\ref{eqn:546}) lead to an approximation which converges and, due to their interpretation as probability densities, ensure positivity. Numerical experiments are used to investigate the closing operator from this section in Chapter~\ref{sec: numerics}.

We close this section with some commentary on the importance of linearity for closing operators.
...
% 	\[\mathbb E[\bs A(t)1(L(t)=\ell, \rapphase(t)=j)]\bs D^{-1} e^{\bs S x}\bs s.\] 
