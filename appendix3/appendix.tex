...
As we did in Appendix~\ref{app:tand}, we treat the cases of no changes, and \(m\geq 1\) changes, of phase from \(\calS_+\) to \(\calS_-\) and \(\calS_-\) to \(\calS_+\) separately. We start with the \emph{no changes} case. The following result is analogous to Lemma~\ref{lem: Dcoajc}, though the proof is somewhat more tedious. 
