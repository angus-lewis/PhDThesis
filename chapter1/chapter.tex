...
In the following, we assume that $\dot W(t) \in [0,\infty)$ and that there is a boundary at level $0$ for both the first and second fluid levels and the first fluid level is also bounded above at \(b\). Thus, the rates of change of the fluid levels can be summarised as:
% 
	\begin{align*} 
...
Now, let ${{\bbpi}}(w) = ( \bbpi_i(w))_{i \in \mathcal{S}}$ be a vector containing the joint limiting density operators and ${\mathbb{p}} = ( {\mathbb p}_i)_{i \in \mathcal{S}}$ be a vector containing the joint limiting mass operators. The determination of ${{\bbpi}}(w)$ involves two important matrices of operators, $ {\mathbb D}$ and $ {\mathbb \Psi}$ which we now introduce. %The operator \( {\mathbb B}\) is the \textit{infinitesimal generator} of the process \(\{(X(t),\varphi(t))\}\). %Intuitively, for a set $\mathcal{E} \subset \mathcal{F}$ and a measure vector $\boldsymbol{\mu} = (\mu_i)_{i \in \mathcal{S}}$, $\bs{\mu}V(t)(\mathcal{E})$, where \(V(t)\) is a semigroup with infinitesimal generator \(B\), gives the conditional probability of $X(t) \in \mathcal{E}$, 
% The operator \( {\mathbb \Psi}\) is such that ${\boldsymbol{\mu}}^{+} {\mathbb \Psi} (\mathcal{E})$ gives the conditional probability of $\{\dot W(t)\}$ returning to level zero and doing so when $X(t) \in \mathcal{E}$, given that the initial distribution of \((X(0),\varphi(0))\) in the set \(\bigcup_{i\in\calS}(\mathcal F_i^+,i)\), is ${\boldsymbol{\mu}}^{+}$. 
...
\subsection{The infinitesimal generator, \(\mathbb D\), of an in-out process}\label{subsec: afjakje}
...
\begin{align*}
%\mathbb{U}^{k \ell}(w,s)&=[\mathbb U_{ij}^{k\ell}(w,s)]_{i\in\calS_k^\bullet,j\in\calS_\ell^\bullet}, \, k,\ell\in \mathcal K^+\cup\mathcal K^-,
...
%	\end{array}\right].\]
%	\begin{align*} 
%		\mathbb{U}(w,s) = \left[\begin{array}{llllll} 
%		\ddots & & & &~\reflectbox{$\ddots$}\\
%            		& \mathbb{U}^{k-1,k-1}(w,s) & \mathbb{U}^{k-1,k}(w,s) & \mathbb{U}^{k-1,k+1}(w,s) \\
%			& \mathbb{U}^{k,k-1}(w,s) & \mathbb{U}^{k,k}(w,s) & \mathbb{U}^{k,k+1}(w,s) \\
%			& \mathbb{U}^{k+1,k-1}(w,s) & \mathbb{U}^{k+1,k}(w,s) & \mathbb{U}^{k+1,k+1}(w,s) \\	
%			\reflectbox{$\ddots$}& & & & \ddots		
%		\end{array}\right], 
%	\end{align*} 
	% 
%	where $\mathbb{U}^{k \ell}=\mathbb U_{ij}^{k\ell}$ is an $|\mathcal{S}| \times |\mathcal{S}|$ matrix of operators for $w > 0$, $s \in \mathbb{C}$, and Re$(s) \geq 0$. 
The matrix of operators \(\mathbb D(s)\) is the infinitesimal generator of the semigroup \(\{\mathbb U(w,s)\}_{w\geq 0}\) defined by 
...
Let $\mathbb\Psi = \mathbb\Psi(0)$. We define $\zeta_W^n(\{0\}) = \inf\{t \geq \zeta_W^{n - 1}(\{0\}): \dot W(t) = 0\}$, for $n \geq  2$, to be the sequence of hitting times to level $0$ of $\dot W(t)$, with $\zeta_W^1(\{0\}): = \zeta_W(\{0\})$. Consider the discrete-time Markov process $\{X({\zeta_W^n(\{0\})}), \varphi(\zeta_W^n(\{0\}))\}_{n \geq 1}$, and for $i \in \mathcal{S},\,k\in\mathcal K_i^-$ define the measures $\bbxi_{k,i}$ given by 
	%
	\begin{align*}
...
We reproduce Theorem 2 of \cite{bo2014} below, which gives the joint limiting distribution of $\{(\dot W(t),X(t),  \varphi(t))\}$. Recall that the joint limiting density operator ${\bbpi}(w) = (\bbpi_i(w))_{i \in \mathcal{S}}$ for $\{(\dot W(t), X(t), \varphi(t))\}$ and the joint limiting mass operator ${\mathbb p} = (\mathbb p_i)_{i \in \mathcal{S}}$ are defined by~\eqref{eqn:jointpi} and \eqref{eqn:jointmass}, respectively. %Following out notational convention, define an equivalent joint limiting density operator $\boldsymbol{\bbpi}(y) = (\bbpi_i(y))_{i \in \mathcal{S}}$ for $\{X(t), W(t), \varphi(t)\}$ and an equivalent joint limiting mass operator $\boldsymbol{\mathbb p} = (\mathbb p_i)_{i \in \mathcal{S}}$, but partitioned accprsing . 
...
\section{Discontinuous Galerkin}\label{sec: dgdgdgdg}
...
% A similar result is established for one-dimensional hyperbolic linear systems in \cite[Section~4.2]{nodalDGBook}. Consider 
% \[\cfrac{\partial \bs u}{\partial t} + \bs A \cfrac{\partial \bs u}{\partial x}=0,\]
% where \(\bs A\) is diagonalizable and we assume that the appropriate boundary conditions are available (see \cite[Section~4.2]{nodalDGBook}).  
